...
def test_voyage_pricing(app):
    ...
    with app.test_client() as test_client:
        ...
        assert data_df[
            (~data_df["ship_imo"].str.contains("NOTFOUND")) & ~(data_df["value_eur"] > 0)
        ].empty
        ...
        prices = data_df.loc[data_df.commodity == base.CRUDE_OIL][
            ["destination_iso2", "commodity", "date", "eur_per_tonne"]
        ]
        prices = prices.drop_duplicates()
        unique_prices = (
            prices.groupby(["destination_iso2", "commodity", "date"])
            .eur_per_tonne.nunique()
            .reset_index()
        )
    ...
    with app.test_client() as test_client:
        ...
        for aggregate_by in aggregate_bys:
            ...
            if "commodity" in aggregate_by:
                expected_columns.update(
                    ["commodity_group", "commodity_group_name", "commodity_name"]
                )

            if "commodity" in aggregate_by:
                assert set(data_df.commodity.unique()) <= set(
                    [
                        base.OIL_PRODUCTS,
                        base.CRUDE_OIL,
                        base.COAL,
...
def test_kazak_oil(app):
    ...
    with app.test_client() as test_client:
        ...
        assert data_df[
            data_df.departure_berth_name.str.startswith("Novorossiysk CPC")
            & (data_df.commodity == "crude_oil")
        ].commodity_origin_iso2.unique().tolist() == ["KZ"]
...
def test_voyage(app):
    ...
    with app.test_client() as test_client:
        ...
        assert all(
            [
                x["arrival_date_utc"] is None or x["arrival_date_utc"] > x["departure_date_utc"]
                for x in data
            ]
        )
        assert all(
            [x["arrival_date_utc"] is not None for x in data if x["status"] == base.COMPLETED]
        )
        ...
        voyage_id, date_berthing = (
            session.query(ShipmentArrivalBerth.shipment_id, Position.date_utc)
...
def test_voyage_rolling(app):
    ...
    with app.test_client() as test_client:
        ...
        params = {
            "format": "json",
            "rolling_days": 7,
            "aggregate_by": "departure_date,destination_country,commodity,status",
        }
        response = test_client.get("/v0/voyage?" + urllib.parse.urlencode(params))
        assert response.status_code == 200
        data = response.json["data"]
...
def test_voyage_companies(app):
    ...
    with app.test_client() as test_client:
        ...
        for company_type in company_types:
            params = {"format": "json", "aggregate_by": "ship_%s" % (company_type)}
            response = test_client.get("/v0/voyage?" + urllib.parse.urlencode(params))
            assert response.status_code == 200
            data = response.json["data"]
            assert len(data) > 0
...
def test_ongoing_coal(app):
    ...
    with app.test_client() as test_client:
        ...
        params = {"commodity": "coal", "status": "ongoing"}
        response = test_client.get("/v0/voyage?" + urllib.parse.urlencode(params))
        assert response.status_code == 200
        data = response.json["data"]
        assert len(data) > 0
