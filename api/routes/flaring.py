...
@routes_api.route('/v0/flaring_facility', strict_slashes=False)
class FlaringFacilityResource(Resource):
    ...
    def get(self):
        ...
        if add_anomaly_index:
            ...
            with engine.connect() as con:
                s = text(sql_content)
                rs = con.execute(s, **{"facility_id": to_list([int(x) for x in facility_id]) if facility_id else None})
    ...
    def build_response(self, result, format, nest_in_data, download):
        ...
        if format == "csv":
            ...
            return Response(
                ...
                headers={"Content-disposition":
                             "attachment; filename=flaringfacility.csv"})
        ...
        if format == "geojson":
            ...
            if download:
                headers = {"Content-disposition":
                               "attachment; filename=flaringfacility.geojson"}
            else:
                headers = {}
...
class FlaringResource(Resource):
    ...
    parser.add_argument('date_from', help='start date for arrival (format 2020-01-15)',
                        default="2018-01-01", required=False)
