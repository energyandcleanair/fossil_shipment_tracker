...
class DepartureResource(Resource):
    ...
    parser.add_argument(
        "unlocode", required=False, help="unlocode(s) of departure port", action="split"
    )
    ...
    def get(self):
        ...
        if format == "csv":
            return Response(
                ...
                mimetype="text/csv",
                headers={"Content-disposition": "attachment; filename=departures.csv"},
            )
    ...
    def roll_average(self, result, aggregate_by, rolling_days):
        ...
        if rolling_days is not None:
            ...
            if date_column is None:
                ...
                result = (
                    result.groupby(
                        [x for x in result.columns if x not in [date_column, "ship_dwt", "count"]],
                        dropna=False,
                    )
                    .apply(
