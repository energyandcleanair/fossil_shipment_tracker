...
@routes_api.route('/v0/portcall', methods=['GET'], strict_slashes=False)
class PortCallResource(Resource):
    ...
    parser.add_argument('check_departure_only', required=False, help='whether it is on the list of checked departures or not',
    ...
    def get(self):
        ...
        if unlocode is not None:
            query = query \
                .filter(Port.unlocode.in_(unlocode))
        ...
        if format == "csv":
            return Response(
                ...
                headers={"Content-disposition":
                             "attachment; filename=portcalls.csv"})

        if format == "json":
            if nest_in_data:
                resp_content = json.dumps({"data": portcalls_df.to_dict(orient="records")}, cls=JsonEncoder)
            else:
