...
class TemplateResource(Resource):
    ...
    parser.add_argument('pivot_value', type=str, help='pivoted value. Default: value_mwh.',
                        required=False, default='value_mwh')
    parser.add_argument('pivot_fill_value', type=float, help='pivot filling value. Default: 0.',
    ...
    def aggregate(self, query, params=None):
        ...
        if any([x not in agg_cols_dict for x in aggregate_by]):
            logger.warning("aggregate_by can only be a selection of %s" % (",".join(agg_cols_dict.keys())))
    ...
    def get_from_params(self, params):
        ...
        # Pivot
        result = self.pivot_result(result=result,
                                   pivot_by=pivot_by,
        ...
        if rolling_days is not None:
            ...
            if not 'date' in result.columns:
                ...
                if len(intersection(['date_without_year','year'], result.columns)) == 2 :
                    ...
                    result[date_column] = pd.to_datetime(year + month_day, format='%Y%m%d', errors='coerce')
                    remove_date = True
                else:
                    # No date information
                    return result
            ...
            result_rolled = result \
                .groupby([x for x in result.columns if x not in (self.date_cols + self.value_cols)]) \
    ...
    def pivot_result(self, result, pivot_by, pivot_value, pivot_fill_value=0):
        ...
        if pivot_by:
            ...
            pivot_by_dependencies = [d for x in to_list(pivot_by) for d in self.pivot_dependencies.get(x,[])]
