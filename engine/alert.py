...
from base.models import (
    ...
    Port,
    AlertConfig,
    AlertCriteriaAssociation,
    AlertRecipient,
    AlertRecipientAssociation,
    Departure,
    Arrival,
)
...
def get_new_alerts():
    ...
    query_shipment2 = (
        session.query(
            ...
            func.lag(query_shipment1.c.destination_iso2)
            .over(
            ...
            func.lag(query_shipment1.c.destination_name)
            .over(
                partition_by=query_shipment1.c.shipment_id,
                order_by=query_shipment1.c.destination_date,
            )
        ...
        func.unnest(
            func.coalesce(
                case(
                    [
                        (
                            query_alert1.c.new_destination_name_pattern == cast([], ARRAY(Text)),
                            sa.null(),
                        )
                    ],
                    else_=query_alert1.c.new_destination_name_pattern,
                ),
                [NULL_ARTIFACT],
            )
        ).label("destination_name_pattern"),
    ...
    query_alert3 = session.query(
        ...
        func.unnest(
            func.coalesce(
                case([(query_alert2.c.commodity == [], sa.null())], else_=query_alert2.c.commodity),
                [NULL_ARTIFACT],
            )
        ).label("commodity"),
    ...
    query_alert4 = session.query(
        ...
        func.unnest(
            func.coalesce(
                case(
                    [(query_alert3.c.departure_port_ids == cast([], ARRAY(BIGINT)), sa.null())],
                    else_=query_alert3.c.departure_port_ids,
                ),
                [NULL_ARTIFACT_BIGINT],
            )
        ).label("departure_port_id"),
    ...
    query_alert_shipment = (
        ...
        .join(
            ...
            sa.and_(
                sa.or_(
                    sa.and_(
                        query_alert.c.destination_iso2 == query_shipment.c.destination_iso2,
                        query_shipment.c.destination_iso2
                        != query_shipment.c.previous_destination_iso2,
                    ),
                ...
                sa.or_(
                    query_alert.c.commodity == query_shipment.c.commodity,
        ...
        .distinct(
            query_alert.c.config_id, query_alert.c.recipient_email, query_shipment.c.shipment_id
        )
    )
    ...
    past_alerts = (
        session.query(
            AlertInstance.config_id,
            AlertInstance.recipient_id,
            func.max(AlertInstance.date_utc).label("last_date_utc"),
        )
        .group_by(AlertInstance.config_id, AlertInstance.recipient_id)
        .subquery()
    )

    query_alert_shipment_new = (
        session.query(query_alert_shipment)
        .outerjoin(
            past_alerts,
            sa.and_(
                past_alerts.c.config_id == query_alert_shipment.c.config_id,
                past_alerts.c.recipient_id == query_alert_shipment.c.recipient_id,
            ),
        )
        .filter(
            sa.or_(
                past_alerts.c.last_date_utc == sa.null(),
...
def build_email_content(mail_alerts_df):
    ...
    content_html = (
        content_html.replace("{config_name}", config_name)
    ...
    content_txt = (
        content_txt.replace("{config_name}", config_name)
        .replace("{config_id}", str(config_id))
...
def build_email_table(alerts_df):
    ...
    table_df = alerts_df.sort_values(["destination_date"], ascending=False).drop_duplicates(
        ["recipient_email", "config_name", "shipment_id"]
    )
    table_df["dwt"] = table_df.dwt.apply(lambda x: "{:,.0f}".format(x))
    table_df["ship"] = (
        table_df.ship_name
        + '<br><span class="commodity">'
        + table_df.commodity_name
        + "</span>"
        + '<br><span class="commodity">'
        + table_df.dwt
        + " tonne</span>"
    ...
    table_df["destination"] = np.where(
        table_df.arrival_port_name.isnull(), table_df.destination, table_df.arrival
    )
    table_df["previous_destination"] = (
        table_df.previous_destination_name + "<br>" + table_df.previous_destination_country
    )

    def imo_to_links(imo):
        return """<a href='https://www.marinetraffic.com/en/ais/details/ships/{imo}' target='_blank'>Marine Traffic</a><br>
                <a href='https://www.vesseltracker.com/en/vessels.html?term={imo}' target='_blank'>Vessel Tracker</a>""".format(
            imo=imo
        ).replace(
            "\n", ""
        )
...
def build_email_contents(alerts_df):
    ...
    :return:
    """
    emails_df = alerts_df.groupby(["recipient_email", "config_id"]).apply(build_email_content)
...
def send_email(recipient_email, shipment_count, content_txt, config_name, content_html, **kwargs):
    ...
    with app.app_context():
        msg = Message(
            "Russia Fossil Tracker: %d new shipment%s for your alert %s"
            % (shipment_count, "s" if shipment_count > 1 else "", config_name),
...
def save_alert(config_id, recipient_id):
    session.add(AlertInstance(config_id=config_id, recipient_id=recipient_id))
    session.commit()
    return
...
def save_alerts(alerts_df):
    ...
    :return:
    """
    alerts_df[["config_id", "recipient_id"]].drop_duplicates().apply(
        lambda row: save_alert(row.config_id, row.recipient_id), axis=1
    )
