...
def test_query_portcall():
    ...
    filtered_portcall, portcalls = Marinetraffic.get_next_portcall(imo="9776755", date_from=date_from,
    ...
    filtered_portcall2, portcalls2 = Marinetraffic.get_next_portcall(imo="9776755", date_from=date_from,
                                                                     arrival_or_departure="arrival", filter=filter)
    assert portcalls2[0].port_id is not None
    assert dt.datetime.strptime(portcalls2[0].date_utc, "%Y-%m-%dT%H:%M:%S") > date_from

    filtered_portcall3, portcalls3 = Marinetraffic.get_next_portcall(imo="9776755", date_from=date_from,
                                                                     arrival_or_departure="arrival", filter=filter,
                                                                     go_backward=True)
    ...
    filtered_portcalls, portcalls = Marinetraffic.get_portcalls_between_dates(
