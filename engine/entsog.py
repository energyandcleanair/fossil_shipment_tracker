...
def get_crossborder_flows(date_from='2022-01-01',
    ...
    if country_iso2:
        ic = ic.loc[(ic.fromCountryKey.isin(to_list(country_iso2)))]

    if partner_iso2:
        ic = ic.loc[(ic.toCountryKey.isin(to_list(partner_iso2)))]
    ...
    flows_import_lng_raw = get_physical_flows(
        operator_key=ic_import_lng.toOperatorKey.to_list(),
        point_key=ic_import_lng.toPointKey.to_list(),
        direction="entry",
        date_from=to_datetime(date_from),
        date_to=to_datetime(date_to),
    ).sort_values(['date'])

    ic_production = ic.loc[ic.fromInfrastructureTypeLabel.notnull() \
                           & ic.fromInfrastructureTypeLabel.str.contains('Production')]

    flows_production_raw = get_physical_flows(
        operator_key=ic_production.toOperatorKey.to_list(),
        point_key=ic_production.toPointKey.to_list(),
        direction="entry",
        date_from=to_datetime(date_from),
        date_to=to_datetime(date_to),
    ).sort_values(['date'])
    ...
    flows_export_lng_raw = get_physical_flows(
        operator_key=ic_export_lng.fromOperatorKey.to_list(),
        point_key=ic_export_lng.fromPointKey.to_list(),
        direction="exit",
        date_from=to_datetime(date_from),
        date_to=to_datetime(date_to),
    ).sort_values(['date'])
    ...
    def process_flows(ic,
        ...
        # Adding LNG
        flows_import_lng = pd.merge(flows_import_lng_raw, points_import,
                                    on=['pointKey', 'operatorKey'],
                                    how='left')
        ...
        # Adding Production
        flows_production = pd.merge(flows_production_raw, points_import,
                                    on=['pointKey', 'operatorKey'],
                                    how='left')
        ...
        flows_export_lng = pd.merge(flows_export_lng_raw, points_export,
                                    on=['pointKey', 'operatorKey'],
                                    how='left')
