...
def update(
    date_from="2015-01-01", date_to=-2, missing_dates_only=True, fill_facilities=False
):
    """
...
class FlaringComputer:
    ...
    def keep_relevant(cls, fires):
        ...
        """
        fires = fires[
            (fires.Temp_BB > cls.min_temp_k) & (fires.Temp_BB < cls.max_temp_k)
        ]
        return fires
    ...
    def spatial_join(cls, flares, geometries):
        ...
        :return:
        """
        flares_gdf = gpd.GeoDataFrame(
            flares, geometry=gpd.points_from_xy(flares.lon, flares.lat)
        )
        flares_gdf.crs = 4326
    ...
    def complete(cls, result, geometries, date):
        ...
        combined = [ids, units]
        merger = pd.DataFrame(
            columns=["id", "unit"], data=list(itertools.product(*combined))
        )
...
class FacilityScraper:
    ...
    def get_fields(cls, gas_only=True):
        ...
        if gas_only:
            ...
            is_gas = [
                "gas.svg" in x["options"]["iconImageHref"] for x in data["features"]
            ]
...
class VnfScraper:
    ...
    def download_vnf_date(cls, date, force=False):
        ...
        r = requests.get(
            data_url, allow_redirects=True, headers={"Authorization": auth}
        )
