...
event_chain as (
	...
	FROM
	    event_chain
	LEFT OUTER JOIN unique_events ON (
    ...
    WHERE
        event_chain.event_to IS NOT NULL
  )
),
-- we take the end of each event chain as a departure for sts, expecting next arrival to be at port
departures_sts AS (
    SELECT
