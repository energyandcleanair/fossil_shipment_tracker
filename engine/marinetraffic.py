...
class Marinetraffic:
    ...
    def get_ship(cls, imo=None, mmsi=None, mt_id=None, use_cache=True):
        ...
        if not response_data:
            ...
            (response_data, response) = cls.call(
    ...
    @classmethod
    def get_portcalls_between_dates(
        cls,
        date_from,
        date_to,
        unlocode=None,
        ...
        (response_datas, response) = cls.call(
            method="portcalls/",
            api_key=api_key,
            params=params,
        ...
        if imo is None:
            ...
            mmsi_imo = (
                session.query(Ship.mmsi, Ship.imo)
                .filter(Ship.mmsi.op("&&")(sa.cast(mmsis, sa.ARRAY(sa.UnicodeText))))
                .all()
            )
            ...
            if imo is None:
                ...
                if not r_imo:
                    ...
                    try:
                        ...
                        is_same = bool(
                            Ship.query.filter(
                                Ship.imo == r_imo[0], Ship.name.any(r["SHIPNAME"])
                            ).count()
                        )
                        if is_same:
                            continue
    ...
    def parse_portcall(cls, response_data):
        data = {
            ...
            "port_id": port.get_id(
                name=response_data["PORT_NAME"],
                unlocode=response_data["UNLOCODE"],
                marinetraffic_id=response_data["PORT_ID"],
            ),
    ...
    def get_next_portcall(
        ...
        for interval in intervals:
            ...
            period_portcalls = cls.get_portcalls_between_dates(
                imo=imo,
                unlocode=unlocode,
    ...
    def get_voyage_info(cls, imo, date_from):
        ...
        if cached_info:
            logger.info(
                "Found a cached VoyageInfo: %s from %s: %s"
                % (imo, date_from, cached_info.destination_name)
            )
            return cached_info
        ...
        # Cache them
        logger.info(
            "Found %d voyage infos for imo %s (%d credits used)"
            % (len(voyageinfos), imo, len(voyageinfos) * 4)
        )
    ...
    def get_closest_position(cls, imo, date, window_hours=24, interval_mins=30):
        ...
        intervals = [
            [
                date - dt.timedelta(minutes=d * interval_mins / 2.0),
                date + dt.timedelta(minutes=d * interval_mins / 2.0),
            ]
    ...
    @classmethod
    def get_ship_events_between_dates(
        cls,
        imo,
        date_from,
        date_to,
