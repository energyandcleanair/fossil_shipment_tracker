...
def get_departures_with_gap_around_arrival(
    ...
    portcall_next = session.query(
        PortCall.id,
        func.lead(PortCall.date_utc)
        .over(partition_by=PortCall.ship_imo, order_by=PortCall.date_utc)
        .label("next_date_utc"),
        func.lead(PortCall.date_utc)
        .over(partition_by=PortCall.ship_imo, order_by=PortCall.date_utc.desc())
        .label("prev_date_utc"),
    ).subquery()

    subq1 = (
        session.query(Departure, portcall_next, Arrival.date_utc.label("arrival_date_utc"))
...
def get_departures_without_arrival(
    min_dwt=None,
    commodities=None,
    date_from=None,
    ship_imo=None,
    ...
    query = (
        session.query(Departure)
        .filter(~Departure.id.in_(subquery))
    ...
    if departure_port_iso2 is not None:
        ...
        query = query.filter(
            sa.or_(Port.iso2.in_(to_list(departure_port_iso2)), Departure.event_id != sa.null())
        )

    return query.order_by(Departure.date_utc)
...
def get_dangling_imo_dates():
    subquery = session.query(Arrival.departure_id)
    return (
        session.query(Departure.ship_imo, Departure.date_utc)
        .filter(~Departure.id.in_(subquery))
        .all()
    )
...
def update(date_from="2022-01-01"):
    ...
    add(
        date_from=date_from,
        unlocode=["RUVYP", "RUULU", "RUMMK", "RULGA", "RUVNN", "RUAZO"],
        commodities=base.GENERAL_CARGO,
    )

    # Only keep oil related for India
    remove(unlocode=["INSIK"], commodities=[base.LNG, base.LPG, base.COAL, base.BULK])

    remove(port_name="SIKKA ANCH", commodities=[base.LNG, base.LPG, base.COAL, base.BULK])

    remove(unlocode=["EGMAH"], commodities=[base.LNG, base.LPG, base.COAL, base.BULK])

    remove(port_name="MERSA EL HAMRA ANCH", commodities=[base.LNG, base.LPG, base.COAL, base.BULK])
...
def add(
    ...
    if marinetraffic_id is not None:
        dangling_portcalls = dangling_portcalls.filter(
...
def remove(
    ...
    if commodities:
        departures = departures.filter(Ship.commodity.in_(to_list(commodities)))

    if unlocode:
        departures = departures.filter(Port.unlocode.in_(to_list(unlocode)))

    if port_id:
        departures = departures.filter(Port.id.in_(to_list(port_id)))

    if marinetraffic_id:
        departures = departures.filter(Port.marinetraffic_id.in_(to_list(marinetraffic_id)))

    if port_name:
        departures = departures.filter(Port.name.in_(to_list(port_name)))

    if date_to:
        departures = departures.filter(Departure.date_utc <= to_datetime(date_to))

    session.query(Departure).filter(Departure.id.in_(departures.scalar_subquery())).delete(
