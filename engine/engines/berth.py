...
def detect_departure_berths(shipment_id=None, min_hours_at_berth=4, max_distance_deg=1):
    ...
    shipments_to_update = session.query(shipments_all.c.shipment_id).filter(
        shipments_all.c.shipment_id.notin_(session.query(ShipmentDepartureBerth.shipment_id))
    )
    ...
    if berths_count.berth_id.max() > 1:
        ...
        berths_agg_ok = berths_agg_ok.sort_values(["shipment_id", "max_date_utc"]).drop_duplicates(
            ["shipment_id"], keep="last"
        )
...
def detect_arrival_berths(shipment_id=None, min_hours_at_berth=4, max_distance_deg=1):
    ...
    shipments_to_update = session.query(shipments_all.c.shipment_id).filter(
        shipments_all.c.shipment_id.notin_(session.query(ShipmentArrivalBerth.shipment_id))
    )
    ...
    if berths_count.berth_id.max() > 1:
        ...
        berths_agg_ok = berths_agg_ok.sort_values(["shipment_id", "min_date_utc"]).drop_duplicates(
