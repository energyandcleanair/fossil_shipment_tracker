...
WITH last_update AS (
    ...
    FROM (
        SELECT
            ...
            kt.product_id,
            MAX(kt.updated_on) OVER (PARTITION BY kz.country_iso2, kt.product_id
                                    ORDER BY DATE(kt.departure_date_utc), kt.updated_on
                                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS updated_on_max
        FROM kpler_trade AS kt
        JOIN kpler_zone AS kz ON kt.departure_zone_id = kz.id
    ) AS sub
    GROUP BY sub.origin_date, sub.country_iso2_departure, sub.product_id
)

UPDATE kpler_trade AS kt
SET is_valid = (kt.is_valid) AND (lu.updated_on_max - kt.updated_on < interval '7 days')
FROM last_update AS lu, kpler_zone AS kz
WHERE kt.departure_zone_id = kz.id
