...
def verify_sync_against_flows(origin_iso2s=None, platforms=None, date_from=None, date_to=None):
    ...
    for country in origin_iso2s:
        for platform in platforms:
            comparison = compare_to_live_flows(
...
def compare_to_live_flows(origin_iso2=None, platform=None, date_from=None, date_to=None):

    dest_comparison = compare_to_live_flows_for_dest(origin_iso2, platform, date_from, date_to)
    product_comparison = compare_to_live_flows_for_product(
...
def compare_to_live_flows_for_dest(origin_iso2, platform, date_from, date_to):

    scraper = KplerFlowScraper()

    flows_from_kpler = scraper.get_flows(
        platform,
    ...
    comparison_per_day = (
        ...
        .aggregate({"problems": "sum", "value_tonne.expected": "sum", "value_tonne.actual": "sum"})
        .reset_index()
    )
...
def compare_to_live_flows_for_product(origin_iso2, platform, date_from, date_to):
