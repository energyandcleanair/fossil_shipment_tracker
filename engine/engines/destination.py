...
def update_matching():
    ...
    dest2 = session.query(
        func.unnest(shipments_all.c.shipment_destination_names).label("destination_name")
    )
    destinations = dest1.union(dest2).subquery()

    new_destinations = (
        session.query(destinations)
    ...
    update = (
        Destination.__table__.update()
        .values(port_id=Port.__table__.c.id, iso2=Port.__table__.c.iso2)
        .where(
            Destination.__table__.c.name != sa.null(),
            or_(
                ...
                func.replace(
                    func.regexp_replace(Destination.__table__.c.name, "(.*)(>|/|_){1,}", ""),
                    " ",
                    "",
                )
                == Port.__table__.c.unlocode,
                func.replace(
                    func.regexp_replace(Destination.__table__.c.name, " VIA(.*)", ""), " ", ""
                )
                == Port.__table__.c.unlocode,
            ),
        )
    ...
    still_missings = (
        Destination.query.join(Shipment, Shipment.last_destination_name == Destination.name)
        .filter(sa.and_(Shipment.status == base.ONGOING, Destination.iso2 == sa.null()))
        .all()
    )
            ...
            ratios = np.array(
                [
                    max(
                        [
                            SequenceMatcher(
                                None, x.name.replace(suf, ""), still_missing.name
                            ).ratio()
    ...
    country_regexps = {
        "RU": [
            ...
            "AZOU$",
            "^PKC$",
            "RUSSIA|RUSNVS$|RU_PGN$|TAUPSE|KAV?KAZ$",
            "^RUS TOCHINO$",
            "^VLDV$",
            "RUAZOV$",
            "RUVFP$",
        ...
        "DK": ["[ |,|_]{1}DENMARK$", "[ |,|_|>]{1}DK$", " SKAW$|SKGEN$|^SKAW$", "DENMARK"],
    ...
    for key, regexps in country_regexps.items():
        ...
        update = Destination.__table__.update().values(iso2=key).where(condition)
        execute_statement(update)
    ...
    update = (
        Destination.__table__.update()
        .values(iso2=country_regex.c.iso2)
        .where(
            sa.and_(
                Destination.iso2 == sa.null(), Destination.name.op("~*")(country_regex.c.regexp)
            )
        )
    )
    logger.info(update)
    ...
    update = Destination.__table__.update().values(iso2=base.FOR_ORDERS).where(condition)
    execute_statement(update)

    # When transit ports only, should be set as null
    unknown_regexps = [
        "BOSP.?ORUS|DRIFTING AREA|GOGLAND|NW EUROPE|GREAT BELT|NW BLACK SEA|NW ?BS|I.?STANBUL|^TR ?IST$"
    ]
    condition = or_(*[Destination.name.op("~")(regexp) for regexp in unknown_regexps])
    update = Destination.__table__.update().values(iso2=sa.null()).where(condition)
    execute_statement(update)
...
def update_from_positions(date_from=None, date_to=None):
    ...
    update = (
        ...
        .values(last_destination_name=shipments_w_last_position.c.destination_name)
    ...
    s1 = (
        session.query(
            ...
            Position.date_utc,
            Position.destination_name,
            func.lag(Position.destination_name)
            .over(partition_by=shipments_all.c.shipment_id, order_by=Position.date_utc)
    ...
    s2 = (
        session.query(s1, Destination.iso2)
        .filter(
            sa.or_(
                s1.c.previous_destination_name == sa.null(),
    ...
    shipments_destinations = (
        session.query(
            s2.c.shipment_id,
            func.array_agg(s2.c.destination_name, type_=ARRAY(String)).label("destination_names"),
            func.array_agg(s2.c.date_utc, type_=ARRAY(String)).label("destination_dates"),
            func.array_agg(s2.c.iso2, type_=ARRAY(String)).label("destination_iso2s"),
        )
        .group_by(s2.c.shipment_id)
        .subquery()
    )
    ...
    update = (
        Shipment.__table__.update()
        .values(
            destination_names=shipments_destinations.c.destination_names,
...
def update_from_voyageinfo(
    ...
    missing_shipments = (
        session.query(Shipment.id, Departure.date_utc, Departure.ship_imo)
    ...
    update = (
        Shipment.__table__.update()
        .values(last_destination_name=new_destinations.c.destination_name)
