CREATE MATERIALIZED VIEW ktc_trade_commodity AS
SELECT
...
FROM
  ...
  LEFT OUTER JOIN ktc_kpler_commodity ON ktc_kpler_commodity.product_id = kpler_product.id
  LEFT OUTER JOIN commodity ON ktc_kpler_commodity.commodity_id = commodity.id
...
CREATE INDEX ON ktc_trade_commodity (trade_id);

CREATE INDEX ON ktc_trade_commodity (flow_id);

CREATE INDEX ON ktc_trade_commodity (pricing_commodity);

ANALYZE ktc_trade_commodity;
