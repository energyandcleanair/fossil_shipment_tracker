CREATE MATERIALIZED VIEW ktc_voyage_insurer_temp AS
SELECT
  DISTINCT ON (
    kpler_trade.id,
    kpler_trade.flow_id,
...
FROM
  ...
  LEFT JOIN ship_insurer ON ship_insurer.ship_imo = ktc_trade_ship_temp.ship_imo
...
CREATE INDEX ON ktc_voyage_insurer_temp (trade_id);

CREATE INDEX ON ktc_voyage_insurer_temp (flow_id);

CREATE INDEX ON ktc_voyage_insurer_temp (ship_imo);

CREATE INDEX ON ktc_voyage_insurer_temp (iso2);

ANALYZE ktc_voyage_insurer_temp;
