...
def update(
    ...
    ship_imo=None,
    commodities=[base.LNG, base.LPG, base.CRUDE_OIL, base.OIL_PRODUCTS, base.OIL_OR_CHEMICAL],
...
def get_and_process_ship_events_between_dates(
    ...
    if not force_rebuild:
        ...
        last_event_call = (
            ...
            .filter(
                MarineTrafficCall.method == base.VESSEL_EVENTS,
    ...
    if force_rebuild and between_existing_only and not between_shipments_only:
        cached_events = session.query(Event).order_by(Event.date_utc)
        ...
        shipment_dates = session.query(
            ...
            sa.case(
                [
                    ...
                    (
                        sa.and_(
                            shipments.c.arrival_date_utc == sa.null(),
                            shipments.c.next_departure_date != sa.null(),
                        ),
                        shipments.c.next_departure_date,
                    ),
                ],
                else_=dt.date.today(),
            ).label("date_to"),
...
def check_distance_between_ships(
    ship_one_imo, ship_two_imo, event_time, window_hours=2, use_cache=True, cache_only=False
):
    """
    ...
    if use_cache:
        ship_position = (
            ...
            .filter(
                Position.ship_imo == ship_one_imo,
                func.abs(func.extract("epoch", Position.date_utc - event_time)) <= (3600.0 * 2),
            )
            .order_by(func.abs(func.extract("epoch", Position.date_utc - event_time)).desc())
            .first()
        )
        intship_position = (
            session.query(Position)
            .filter(
                Position.ship_imo == ship_two_imo,
                func.abs(func.extract("epoch", Position.date_utc - event_time)) <= (3600.0 * 2),
            )
            .order_by(func.abs(func.extract("epoch", Position.date_utc - event_time)).desc())
            .first()
    ...
    if ship_position is None and not cache_only:
        ship_position = Marinetraffic.get_closest_position(
            imo=ship_one_imo, date=event_time, window_hours=window_hours, interval_mins=5
        )
    if intship_position is None and ship_position is not None and not cache_only:
        intship_position = Marinetraffic.get_closest_position(
            imo=ship_two_imo, date=event_time, window_hours=window_hours, interval_mins=5
        )
    ...
    time_difference = 0.5 + abs(
        (ship_position.date_utc - intship_position.date_utc).total_seconds() / 3600.0
    )

    return ship_position_geom, intship_position_geom, d, time_difference
...
def find_ships_in_db(ship_name):
    ...
    ships = (
        session.query(Ship)
        .filter(
            sa.or_(Ship.name.any(ship_name), Ship.name.any(func.lower(ship_name))),
            Ship.dwt > base.DWT_MIN,
            ~Ship.imo.contains("NOTFOUND"),
        )
        .all()
    )
...
def add_interacting_ship_details_to_event(event, distance_check=30000):
    ...
    ship_imo, event_content, ship_name, event_time = (
        event.ship_imo,
        event.content,
        event.ship_name,
        event.date_utc,
    ...
    if int_ships:
        for intship in int_ships:
            ship_position, intship_position, d, position_time_diff = check_distance_between_ships(
                ship_imo, intship.imo, event_time
            )
    ...
    if int_ships is None:
        ...
        if int_ship_imo_local is not None:
            event.interacting_ship_imo = int_ship_imo_local
...
def back_fill_ship_position(
    ...
    events = session.query(Event).filter(
        Event.interacting_ship_imo != sa.null(), Event.ship_closest_position == sa.null()
    )
    ...
    for e in tqdm(events.all()):
        if not force_rebuild:
            ...
            matches = [
                q
                for q in previous_calls
                if to_datetime(q.fromdate) <= to_datetime(e.date_utc) <= to_datetime(q.todate)
            ]
        ...
        ship_position, intship_position, d, position_time_diff = check_distance_between_ships(
            e.ship_imo, e.interacting_ship_imo, e.date_utc, use_cache=True, cache_only=False
        )
